...
export function GeminiSettingsPanel() {
  ...
  async function refreshModels() {
    ...
    try {
      const result = await geminiApi<{ models: GeminiModelOption[] }>({ action: 'models', apiKey });
  ...
  return (
    <form onSubmit={save} className="rounded-xl border border-[var(--sp-line)] bg-[var(--sp-surface)] p-5 md:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex gap-3">
          <Sparkles className="mt-0.5 size-5 shrink-0 text-[var(--sp-brand)]" aria-hidden="true" />
          <div>
            <h2 className="font-extended text-lg font-bold">Переводы с Gemini</h2>
            <p className="mt-1 max-w-2xl text-xs leading-5 text-[var(--sp-ink-tertiary)]">
              Заполняет русскую, узбекскую и английскую версии названий и описаний. Готовый текст всегда можно изменить вручную.
            </p>
          </div>
        </div>
        <label className="inline-flex min-h-10 shrink-0 cursor-pointer items-center gap-2 rounded-lg border border-[var(--sp-line)] px-3 text-xs font-bold">
      ...
      {error || notice ? <p role={error ? 'alert' : 'status'} className={`mt-5 rounded-lg border px-4 py-3 text-sm ${error ? 'border-red-300/50 bg-red-500/8 text-[var(--sp-danger)]' : 'border-emerald-400/30 bg-emerald-500/8 text-[var(--sp-success)]'}`}>{error || notice}</p> : null}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="text-xs font-bold">
          ...
          <span className="relative block">
            ...
            />
            <button type="button" onClick={() => setShowKey((visible) => !visible)} aria-label={showKey ? 'Скрыть API-ключ' : 'Показать API-ключ'} className="absolute right-1.5 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-md text-[var(--sp-ink-tertiary)] hover:bg-[var(--sp-surface-inset)] hover:text-[var(--sp-ink)]">
      ...
      <div className="mt-4 max-w-[calc(50%-0.5rem)] max-md:max-w-none">
        <CustomSelect
          label="Модель для визуализации пакетов"
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:justify-end">
        ...
        <button type="submit" disabled={busy || (!apiKey && !settings.apiKeyConfigured)} className="min-h-11 rounded-lg bg-[var(--sp-brand)] px-6 text-xs font-bold text-[var(--sp-on-brand)] disabled:opacity-40">
          {busy ? 'Сохраняем…' : 'Сохранить Gemini'}
        </button>
      </div>
    </form>
  );
