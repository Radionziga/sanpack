...
import { getCategoryDepth, getProductsInCategoryScope, getVisibleCategories } from '@/lib/catalog/categoryHierarchy';
...
export default async function HomePage({
  ...
  const activeCategories = getVisibleCategories(categories)
    .sort((a, b) => a.sortOrder - b.sortOrder);
  const categoryProductCounts = Object.fromEntries(activeCategories.map((category) => {
    return [category.id, getProductsInCategoryScope(publishedProducts, activeCategories, category.id).length];
  }));
  const shelfCategories = activeCategories
    .filter((category) => getCategoryDepth(category.id, activeCategories) === 1 && categoryProductCounts[category.id] > 0)
    .slice(0, 10);
  const homeProductIds = new Set(shelfCategories.flatMap((category) => getProductsInCategoryScope(publishedProducts, activeCategories, category.id)
    .slice(0, 6)
