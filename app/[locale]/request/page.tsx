            ...
            {items.length === 0 ? (
              ...
              <form ref={formRef} onSubmit={submit} className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
                ...
                <aside className="rounded-[var(--sp-radius-lg)] border border-[var(--sp-line)] bg-[var(--sp-surface)] p-5 shadow-[var(--sp-shadow-raised)] lg:sticky lg:top-6">
                  ...
                  <label className="mt-5 block text-xs font-medium">Имя<span className="mt-2 flex min-h-12 items-center gap-2 rounded-[var(--sp-radius-control)] border border-[var(--sp-line-strong)] bg-[var(--sp-control)] px-3 focus-within:border-[var(--sp-brand)]"><User className="size-4 text-[var(--sp-ink-muted)]" aria-hidden="true" /><input required minLength={2} maxLength={120} value={contactName} onChange={(event) => setContactName(event.target.value)} autoComplete="name" className="w-full bg-transparent py-3 text-sm outline-none" placeholder="Как к вам обращаться" /></span></label>
                  <label className="mt-4 block text-xs font-medium">Телефон<span className="mt-2 flex min-h-12 items-center gap-2 rounded-[var(--sp-radius-control)] border border-[var(--sp-line-strong)] bg-[var(--sp-control)] px-3 focus-within:border-[var(--sp-brand)]"><Phone className="size-4 text-[var(--sp-ink-muted)]" aria-hidden="true" /><input required inputMode="tel" value={phone} onChange={(event) => setPhone(event.target.value)} autoComplete="tel" className="w-full bg-transparent py-3 text-sm outline-none" placeholder="+998 90 123 45 67" /></span></label>
                  ...
                  <button type="submit" disabled={isSubmitting || authLoading} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[var(--sp-radius-control)] bg-[var(--sp-brand)] px-5 text-xs font-medium text-[var(--sp-on-brand)] disabled:cursor-wait disabled:opacity-60">{!customer.authenticated ? <Send className="size-4" /> : null}{isSubmitting ? 'Отправляем…' : authLoading ? 'Проверяем вход…' : customer.authenticated ? 'Отправить заявку' : 'Войти через Telegram и оформить'}</button>
