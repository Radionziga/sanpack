...
export default function IntegrationsPage() {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setLoginSecret(''); setStorefrontToken(''); setNotificationToken(''); setNotice(result.message);
    } catch (reason) {
      ...
      <form onSubmit={save} className="space-y-5">
        <section className="rounded-xl border border-[var(--sp-line)] bg-[var(--sp-surface)] p-5 md:p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <input type="checkbox" checked={settings.login.enabled} onChange={(event) => setSettings((current) => ({ ...current, login: { ...current.login, enabled: event.target.checked } }))} className="size-5 accent-[var(--sp-brand)]" aria-label="Включить вход через Telegram" />
          </div>
        ...
        <section className="rounded-xl border border-[var(--sp-line)] bg-[var(--sp-surface)] p-5 md:p-6">
          <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><Bot className="mt-0.5 size-5 text-[var(--sp-brand)]" /><div><h2 className="font-extended text-lg font-bold">Бот магазина — Mini App</h2><p className="mt-1 text-xs leading-5 text-[var(--sp-ink-tertiary)]">Открывает витрину внутри Telegram.</p></div></div><input type="checkbox" checked={settings.storefront.enabled} onChange={(event) => setSettings((current) => ({ ...current, storefront: { ...current.storefront, enabled: event.target.checked } }))} className="size-5 accent-[var(--sp-brand)]" aria-label="Включить Mini App" /></div>
        ...
        <section className="rounded-xl border border-[var(--sp-line)] bg-[var(--sp-surface)] p-5 md:p-6">
          <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><Send className="mt-0.5 size-5 text-[var(--sp-brand)]" /><div><h2 className="font-extended text-lg font-bold">Уведомления администратору</h2><p className="mt-1 text-xs leading-5 text-[var(--sp-ink-tertiary)]">После сохранения заявки бот отправит сообщение в выбранный чат.</p></div></div><input type="checkbox" checked={settings.notifications.enabled} onChange={(event) => setSettings((current) => ({ ...current, notifications: { ...current.notifications, enabled: event.target.checked } }))} className="size-5 accent-[var(--sp-brand)]" aria-label="Включить уведомления" /></div>
