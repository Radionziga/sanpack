  ...
  async function saveOrder() {
    ...
    try {
      const updated = await AdminRepository.updateRequest(selected.id, {
        ...
        deliveryWindow: selected.deliveryWindow || '',
      ...
      {selected ? <div className="fixed inset-0 z-50 bg-black/55 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={`Заказ ${selected.requestNumber}`}><div className="mx-auto flex max-h-full w-full max-w-5xl flex-col overflow-hidden rounded-xl border border-[var(--sp-line)] bg-[var(--sp-canvas)] shadow-2xl">
        ...
        <div className="grid min-h-0 flex-1 overflow-y-auto lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="space-y-5 p-5 sm:p-6">
            ...
            <section className="admin-panel-muted grid gap-4 p-4 sm:grid-cols-2">
              <label className="text-xs font-bold sm:col-span-2">Адрес доставки<input value={selected.deliveryAddress || ''} onChange={(event) => patchSelected({ deliveryAddress: event.target.value })} className="admin-control mt-2 text-sm" placeholder="Адрес не указан" /></label>
              <label className="text-xs font-bold">Дата доставки<input type="date" value={selected.deliveryDate || ''} onChange={(event) => patchSelected({ deliveryDate: event.target.value })} className="admin-control mt-2 text-sm" /></label>
              <label className="text-xs font-bold">Интервал доставки<CustomSelect value={selected.deliveryWindow || ''} onChange={(value) => patchSelected({ deliveryWindow: value })} options={[{ value: '', label: 'Не указан' }, { value: '09:00-13:00', label: '09:00–13:00' }, { value: '13:00-17:00', label: '13:00–17:00' }, { value: '17:00-21:00', label: '17:00–21:00' }]} size="sm" ariaLabel="Интервал доставки" className="mt-2" /></label>
            </section>
