...
export default function DocumentSettingsPage() {
  ...
  async function save(event: FormEvent) {
    ...
    try { const response = await fetch('/api/admin/document-settings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(settings) }); const body = await response.json(); if (!response.ok) throw new Error(body.error); setSettings(body); setMessage('Настройки документов сохранены.'); }
