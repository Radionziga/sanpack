...
Первый подтверждённый вход автоматически создаёт `customers` profile. Повторный вход обновляет provider metadata, но сохраняет отредактированные покупателем `name`/`phone`. Общий resolver сохраняет найденный legacy UID как primary и всегда включает детерминированный `telegram:<verified id>` в bounded signed aliases, даже если отдельного canonical document нет. Proof-only Mini App checkout обязан вызывать resolver; history и idempotency могут принимать другой UID только из server-derived aliases того же Telegram account.

Новые HttpOnly customer cookies имеют случайный `sessionId` и server-side запись `customerSessions`. Protected API проверяет UID, Telegram ID, revocation и `expiresAt`. Profile write и refresh active record происходят одной Firestore transaction; update-only refresh не может воскресить удалённую logout-сессию. Logout fail-closed: success возвращается только после подтверждённого delete. Cookies старого формата переходно принимаются до собственного JWT expiry, не продлеваются PUT и при logout очищаются только локально. Bridge удаляется после 30 полных дней без revision, способной выпускать старый формат; rollback на старую revision начинает отсчёт заново.

Mini App bootstrap имеет три результата: обычный browser без `initData`, authenticated либо rejected. При rejected proof UI не читает cookie-backed Profile/History и блокирует Checkout, предотвращая показ предыдущего browser account. Private integration settings находятся вне SiteSettings. Backoffice documents используют `backofficeSettings/documents` и отдельный order document API.
...
Locale layout получает SiteSettings для default metadata/company identity/favicon. `lib/seo/metadata.ts` содержит route-level primitives, а `lib/seo/policy.ts` централизует effective Product/Category metadata: exact localized override → localized entity data → reviewed deterministic locale template. Product/category layouts, Admin preview и sitemap eligibility используют одну policy; generated SEO не записывается в Firestore. Основные content routes (`about`, `clients`, `delivery`, `branding`, `contacts`, `privacy`, `terms`) и Link Hub имеют собственную metadata вместо наследования Home. Product JSON-LD — только реальный sale Offer (см. pricing), locale layout добавляет Organization/WebSite, Product и taxonomy pages — BreadcrumbList. Dynamic sitemap сочетает static paths, включённый Link Hub, active taxonomy lineage и published products в visible Category/Subcategory для четырёх локалей; при backend failure dynamic lists пусты, static routes остаются, seed автоматически не включается. Developer contract: [SEO_BY_DEFAULT](SEO_BY_DEFAULT.md).

Indexability: published Product indexable только при valid slug и visible taxonomy lineage; active taxonomy indexable только при active lineage. Draft/hidden entities и Products внутри hidden/broken lineage не попадают в sitemap. Admin имеет layout noindex. Search/favorites/request/orders/profile/print и API получают `X-Robots-Tag: noindex, nofollow`; robots дополнительно исключает их из discovery. Filter combinations не являются SEO routes и не создают индексируемые faceted URLs.
...
## Excel Price Manager — 2026-09-20

`/admin/prices` is an owner-only bulk editor for existing base Product prices and existing explicit Variant price overrides. It is deliberately narrower than the Product editor: price mode, wholesale tiers, packaging, stock, titles, publication and Product creation are outside this workflow. `pricing.write` is enforced both in navigation/page composition and every server route.

Export reads current Products through the Admin SDK and creates a styled `.xlsx` plus a short-lived server manifest in `priceExportManifests`. The workbook carries stable row identities and a hidden integrity sheet; editable cells never become authority by themselves. Import accepts bounded macro-free `.xlsx`, resolves every row against the server manifest and current Product, and stores only a preview/audit batch in `priceImportBatches`. Product documents are not written during upload.

Workbook schema v2 groups rows into one visible sheet per direct `Product.categoryId`, ordered by the existing taxonomy. Parent scope is never expanded, so one Product/Variant identity occurs in exactly one category sheet. The server manifest signs the expected sheet/category mapping; renamed, missing, added sheets and rows moved between categories are structural errors. Schema v1 exports that have not expired remain readable through their legacy single `Цены` sheet, without weakening v2 validation.

Explicit apply revalidates every target inside a Firestore transaction, patches only base `price` or the selected Variant `price`, and is idempotent per batch. A stale price or price-mode change blocks the whole operation. Rollback is another conflict-checked transaction and succeeds only while every affected target still equals the value written by that batch. Direct browser Firestore access remains denied. Export manifests expire after 90 days through Firestore TTL; import history is retained as the operational audit trail.
