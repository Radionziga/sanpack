...
Commercial UX Phase 1 **2026-09-12** сохраняет B2B request semantics и единый pricing engine. Общий presentation helper различает полностью priced, mixed и request-only состав: mixed subtotal никогда не маскирует строки «по запросу», request-only не показывает искусственный ноль. Product CTA показывает sales unit, упаковку, фактический минимум покупки и wholesale threshold; количество можно вводить числом с нормализацией существующими min/step/max rules. Request composition и line totals идут перед контактами, Variant SKU участвует в storefront search с подписью совпавшего варианта. Mobile navigation немедленно показывает pending state, а Mini App скрывает бессмысленный logout, поскольку account switching управляется Telegram. Фактический handoff: [UX_COMMERCIAL_PHASE1_HANDOFF_2026-09-12.md](UX_COMMERCIAL_PHASE1_HANDOFF_2026-09-12.md).

Product UX Phase 2 **2026-09-13** не меняет catalog/commerce model. Catalog использует компактную category rail и показывает максимум 24 Product cards при первом render; последующие страницы раскрываются явной кнопкой, а page/sort/filter state живёт в URL. Search различает loading/error/empty, сохраняет query и scroll при Product → Back; совпавший Variant SKU передаётся как optional query на canonical Product route и сразу выбирается в существующем editor. История заявок может восстановить состав только после сверки с текущими Product/Variant/price/quantity/packaging semantics. Это наполнение корзины, не автоматическая отправка заявки; существующая корзина merge-ится по Product+Variant. Новые Request snapshots получают optional `orderRule`, старые документы остаются совместимыми без migration. Фактический handoff: [PRODUCT_UX_PHASE2_HANDOFF_2026-09-13.md](PRODUCT_UX_PHASE2_HANDOFF_2026-09-13.md).

First-party product analytics **2026-09-13** использует отдельные случайные HttpOnly visitor/session IDs и никогда не связывает их с Telegram/customer UID или контактными данными. Storefront отправляет только типизированные bounded engagement events в trusted server endpoint; подтверждённая conversion `request_created` создаётся только сервером после успешной заявки и дедуплицируется по Request ID. Admin получает агрегированный DTO через `/api/admin/analytics`, доступный только capability `analytics.read` (первоначально `super_admin`). Session idle window — 30 минут, business timezone — `Asia/Tashkent`, dashboard period — до 90 дней; raw event/session retention — 190 дней, visitor retention — 400 дней через scoped Firestore TTL. Прямой Firestore client access остаётся deny-all. Подробности: [FIRST_PARTY_ANALYTICS_HANDOFF_2026-09-13.md](FIRST_PARTY_ANALYTICS_HANDOFF_2026-09-13.md).

External analytics — только дополнительная presentation/delivery boundary поверх этой first-party taxonomy, а не новый источник истины. GA4 получает явные SPA page views и bounded product/cart/lead events; Яндекс Метрика использует явные hits/goals с выключенными Webvisor, clickmap, form/session recording. Успешная B2B-заявка отображается как `generate_lead`/`request_created`, никогда как `purchase`. Свободный текст поиска, customer/Telegram identity, контакты, адрес, комментарий и Request payload внешним провайдерам не передаются. Общий privacy opt-out, DNT/GPC, localhost и automation отключают внешние scripts; прежний HttpOnly opt-out учитывается на server layout. Настройка public provider IDs живёт в существующем `SiteSettings`, Admin → Integrations; секреты не нужны. Operational record: [EXTERNAL_ANALYTICS_SEARCH_HANDOFF_2026-09-14.md](EXTERNAL_ANALYTICS_SEARCH_HANDOFF_2026-09-14.md).

Category artwork остаётся двумя независимыми presentation slots: `navigationImage` для маленькой светлой иконки sidebar и `bentoImage` для широкой Home-card. Исторические hardcoded artwork maps сохранены как явный built-in fallback без migration; Admin теперь показывает оба фактически используемых изображения и источник fallback, поэтому скрытых runtime-картинок больше нет. Поле `image` — только legacy fallback.
...
Через `SiteSettings` меняются name, logos/dark logo/favicon, descriptions, contacts, theme tokens/font preset, default SEO, сервисная навигация и optional Link Hub. `/[locale]/links` — самостоятельная мобильная brand/contact страница без общего Header/Footer/bottom navigation; `/admin/links` управляет локализованными текстами, highlight и упорядоченными ссылками. Допустимы только внутренние paths, HTTPS, `tel:` и `mailto:`; public projection остаётся explicit allowlist. Старые settings documents получают defaults без migration. Generic floating contact использует company name. `ownProduction` остаётся флагом, фильтр скрыт, если в текущем scope нет таких товаров; это не новая tags subsystem.
...
- Search не индексирует attributes. Product/Variant SKU, localized Product/Variant names и Category names поддерживаются; совпавший Variant подписывается и выбирается при переходе на canonical Product route через optional query.
...
- Public read получает коллекции целиком; client filtering/search — не индекс для огромного каталога. Initial Catalog DOM теперь bounded (24 cards), но trusted-server Product payload всё ещё полный и при заметном росте ассортимента потребует отдельного измеренного listing projection/backend-search этапа. Runtime IAM/secrets и TTL проверены при rollout; alerts и cost dashboards остаются эксплуатационной задачей, а не свойством codebase.
...
## Product UX Phase 2 production release — 2026-09-13

Link Hub and Product UX Phase 2 are live from source `d0a048dde44de0a8ada3fa9df414c760d5c98e67` after successful automatic App Hosting rollout `fah-stamply-4df8a-sanpack-rollout-2026-09-13-002`. Catalog uses compact navigation and a bounded 24-card initial render; Search preserves query/result/scroll context through Product → Back; Customer History can restore a prior composition only after current-catalog reconciliation and never submits a Request automatically. Link Hub is localized and Admin-configurable; its top-left language menu opens into the viewport on narrow embedded browsers.

Production smoke confirmed 238/238 public Products have image URLs, initial/load-more pagination works, exact Variant SKU context survives navigation, critical locale routes respond, and Link Hub has no 320–430 px horizontal overflow. No Request, Telegram notification, production content/data, taxonomy, security rules, IAM or secrets were changed. The local public-catalog mirror is development-only and exists solely to avoid misleading incomplete seed media during local visual verification. Detailed release record: [PRODUCT_UX_PHASE2_HANDOFF_2026-09-13.md](PRODUCT_UX_PHASE2_HANDOFF_2026-09-13.md).

## First-party analytics production release — 2026-09-13

Privacy-conscious product analytics is live on source `c149d361bf47b9a76152a3755d5b0cf2a66a53a5`, App Hosting revision `sanpack-build-2026-09-13-004`, 100% traffic. Collection started at 2026-09-13 16:59:28 UTC (21:59:28 Asia/Tashkent); there is no inferred history before launch. Storefront engagement writes through a typed, rate-limited server endpoint; `request_created` is server-confirmed and idempotent. Admin `/admin/analytics` exposes only aggregate KPIs, funnel, products, pages and campaign/source breakdowns under `analytics.read`; raw IDs and customer PII are not exposed or linked.

TTL is `ACTIVE` for analytics events/sessions/visitors. Firestore/Storage rules remain unchanged and direct client access stays denied. Category Admin also makes Home curation explicit: every non-group category shows its Home status, with a separate enable switch and numeric position; sidebar icon and Home cover remain independent. No production category values were changed. Full evidence and operational limits: [FIRST_PARTY_ANALYTICS_HANDOFF_2026-09-13.md](FIRST_PARTY_ANALYTICS_HANDOFF_2026-09-13.md).

## Real production catalog taxonomy — 2026-09-14

Production now uses the existing three-level model as a buyer-facing hierarchy: four commercial Categories (`Пакеты и мешки`, `Мясо, птица и яйца`, `Бакалея и ингредиенты`, `Овощи, фрукты и зелень`) group 19 existing leaves as Subcategories. All 238 Product IDs, SKUs, Product category IDs, prices, packaging rules, publication states, media references, request history and analytics identity were preserved. Six Products whose image previously came from a compatibility map now explicitly reference that same existing asset.

The deterministic mapping, private backup contract and verification are in [REAL_CATALOG_TAXONOMY_HANDOFF_2026-09-14.md](REAL_CATALOG_TAXONOMY_HANDOFF_2026-09-14.md). Do not flatten the tree, clone Products or regenerate the mapping through fuzzy title matching. Future taxonomy changes go through the existing Admin CMS. Parent category names are searchable through the existing lineage helper; an otherwise non-actionable one-value facet is hidden unless it is active.

The application support patch is live from source `dcd1c8a5cb7093b7ca7363f20c46aed3e8a11681` as `sanpack-build-2026-09-14-001`, 100% traffic; rollout `rollout-2026-09-14-001` succeeded at `2026-09-14T03:27:20.765860Z`. Immediate application rollback is `sanpack-build-2026-09-13-005`; the content rollback uses the private exact-document backup recorded in the handoff. Post-cutover health, search, routes, mobile presentation, Admin and Analytics passed without a Request, Telegram notification or customer write.

## Excel Price Manager release candidate — 2026-09-20

`/admin/prices` adds a bounded, owner-only Excel workflow for changing existing numeric base Product prices and existing explicit Variant price overrides. Export, strict preview, explicit confirmation, atomic apply, audit history and conflict-safe rollback share the `pricing.write` server capability. Upload never writes Products; stale catalog state blocks the whole batch; large changes above 30% require a separate acknowledgement. Price modes, wholesale tiers, packaging, publication and unrelated Product fields are outside the workflow.

Collections are server-only: `priceExportManifests` is a 90-day TTL identity snapshot and `priceImportBatches` is the operational audit record. Direct Firestore access remains denied. The release gate and rollout evidence are recorded in [EXCEL_PRICE_MANAGER_HANDOFF_2026-09-20.md](EXCEL_PRICE_MANAGER_HANDOFF_2026-09-20.md).

Workbook schema v2 replaces the duplicated master-table pattern with direct-category sheets in taxonomy order. Each exported row carries its direct category identity and expected sheet in the server manifest. The import parser normalizes all category sheets back into the existing canonical row set before preview/apply; it rejects renamed, missing, added sheets, duplicate identities and cross-category row movement. Existing pricing, transaction, history and rollback contracts remain unchanged.
