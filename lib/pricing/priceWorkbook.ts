...
export async function createPriceWorkbook(input: {
  ...
  const instruction = workbook.addWorksheet('Инструкция', { views: [{ showGridLines: false }] });
  ...
  instruction.getCell('C3').value = workbookDate(input.createdAt);
  ...
  const steps = [
    ...
    'Не удаляйте и не добавляйте строки, не меняйте SKU и служебные данные.',
    'Сохраните файл в формате .xlsx.',
    'Загрузите файл в Admin → Цены.',
    'Проверьте найденные изменения и предупреждения.',
  ...
  const prices = workbook.addWorksheet(PRICE_SHEET, {
    views: [{ state: 'frozen', xSplit: 5, ySplit: HEADER_ROW, topLeftCell: 'F5', showGridLines: false }],
  });
  prices.getCell('A1').value = 'Актуальные базовые цены SANPACK';
  prices.getCell('A1').font = { name: 'Arial', size: 14, bold: true, color: { argb: 'FF0A4B2E' } };
  prices.mergeCells('A1:F1');
  prices.getCell('A2').value = `Экспорт: ${workbookDate(input.createdAt)} · ${rowsLabel(input.rows.length)}`;
  prices.getCell('A2').font = { name: 'Arial', size: 9, color: { argb: 'FF4C5751' } };
  prices.mergeCells('A2:F2');
  prices.getCell('K2').value = 'Редактируйте только зелёные ячейки →';
  prices.getCell('K2').font = { name: 'Arial', size: 9, bold: true, color: { argb: 'FF0F6E43' } };
  prices.mergeCells('K2:L2');

  const tableRows = input.rows.map((row, index) => {
    const excelRow = FIRST_DATA_ROW + index;
    return [
      textOrBlank(row.group), textOrBlank(row.category), textOrBlank(row.subcategory), safeText(row.sku),
      safeText(row.productTitle), textOrBlank(row.variantTitle), row.priceLevel, safeText(row.salesUnit),
      row.priceMode, row.priceSource, row.currentPrice, row.editable ? row.exportedPrice : null,
      { formula: `IF(OR(K${excelRow}="",L${excelRow}=""),"",L${excelRow}-K${excelRow})` },
      { formula: `IF(OR(K${excelRow}="",L${excelRow}="",K${excelRow}=0),"",(L${excelRow}-K${excelRow})/K${excelRow})` },
      row.rowId, row.productId, row.variantId || null, row.priceOwner, row.editable, row.exportedPrice,
      PRICE_WORKBOOK_SCHEMA_VERSION,
    ];
  });
  prices.addTable({
    name: 'SanpackPriceRows',
    ref: `A${HEADER_ROW}`,
    headerRow: true,
    totalsRow: false,
  ...
  prices.getRow(HEADER_ROW).height = 34;
  prices.getRow(HEADER_ROW).font = { name: 'Arial', size: 9, bold: true, color: { argb: 'FFFFFFFF' } };
  prices.getRow(HEADER_ROW).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  prices.columns.forEach((column) => { column.font = { name: 'Arial', size: 9, color: { argb: 'FF151B18' } }; });
  const widths = [18, 22, 22, 16, 34, 25, 14, 15, 16, 18, 16, 16, 16, 14];
  widths.forEach((width, index) => { prices.getColumn(index + 1).width = width; });
  for (let index = 15; index <= 21; index += 1) prices.getColumn(index).hidden = true;
  prices.getColumn(11).numFmt = '#,##0" сум"';
  prices.getColumn(12).numFmt = '#,##0" сум"';
  prices.getColumn(13).numFmt = '+#,##0" сум";-#,##0" сум";0" сум"';
  prices.getColumn(14).numFmt = '+0.0%;-0.0%;0.0%';
  prices.getColumn(4).alignment = { vertical: 'middle' };
  prices.getColumn(5).alignment = { vertical: 'middle', wrapText: true };
  prices.getColumn(6).alignment = { vertical: 'middle', wrapText: true };
  prices.getColumn(11).alignment = prices.getColumn(12).alignment = prices.getColumn(13).alignment = prices.getColumn(14).alignment = { horizontal: 'right', vertical: 'middle' };
  input.rows.forEach((row, index) => {
    const excelRow = FIRST_DATA_ROW + index;
    prices.getRow(excelRow).height = 30;
    const newPrice = prices.getCell(excelRow, 12);
    if (row.editable) {
  ...
  const lastRow = Math.max(FIRST_DATA_ROW, FIRST_DATA_ROW + input.rows.length - 1);
  prices.addConditionalFormatting({ ref: `N${FIRST_DATA_ROW}:N${lastRow}`, rules: [
  ...
  await prices.protect(SHEET_PASSWORD, {
...
export async function parsePriceWorkbook(buffer: Buffer) {
  await inspectXlsxArchive(buffer);
  const workbook = new ExcelJS.Workbook();
  // ExcelJS declares its browser-compatible Buffer as ArrayBuffer. Copying the
  // Node buffer avoids passing a potentially shared backing store to the parser.
  await workbook.xlsx.load(Uint8Array.from(buffer).buffer);
  if (workbook.worksheets.length > 5) throw new Error('В Excel слишком много листов. Используйте файл, скачанный из SANPACK.');
  const prices = workbook.getWorksheet(PRICE_SHEET);
  const meta = workbook.getWorksheet(META_SHEET);
  if (!prices || !meta) throw new Error('Не найдены обязательные листы «Цены» и служебные данные. Скачайте новый Excel.');
  if (prices.rowCount > PRICE_WORKBOOK_MAX_ROWS || prices.columnCount > PRICE_WORKBOOK_MAX_COLUMNS) {
    throw new Error('Excel превышает допустимый размер таблицы.');
  }

  const metadata = new Map<string, string>();
  meta.eachRow((row, rowNumber) => {
    if (rowNumber > 1) metadata.set(cellText(row.getCell(1), 80), cellText(row.getCell(2), 500));
  });
  const schemaVersion = Number(metadata.get('schemaVersion'));
  if (schemaVersion !== PRICE_WORKBOOK_SCHEMA_VERSION) throw new Error('Версия Excel не поддерживается. Скачайте актуальный файл.');
  ...
  prices.getRow(HEADER_ROW).eachCell((cell, column) => headerMap.set(cellText(cell, 80), column));
  for (const header of priceWorkbookHeaders) {
    if (!headerMap.has(header)) throw new Error(`В таблице отсутствует колонка «${header.startsWith('__') ? 'служебные данные' : header}».`);
  }
  const column = (header: typeof priceWorkbookHeaders[number]) => headerMap.get(header)!;
  const rows: ParsedPriceWorkbookRow[] = [];
  for (let rowNumber = FIRST_DATA_ROW; rowNumber <= prices.rowCount; rowNumber += 1) {
    const row = prices.getRow(rowNumber);
  ...
  return { exportId, schemaVersion, catalogDigest, rows };
}
